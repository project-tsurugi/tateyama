...
perf_stat() {
	session_num=$1
	echo "perf stat -e ${PERF_EVENTS} -x , ${APP} ${session_num} ${CLIENT_TYPE} ${MSG_LEN} ${NLOOP} ${COMM_TYPE}"
	perf stat -e ${PERF_EVENTS} -x , ${APP} ${session_num} ${CLIENT_TYPE} ${MSG_LEN} ${NLOOP} ${COMM_TYPE}
	echo ""
	sleep 3
}
