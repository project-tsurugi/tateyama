...
|default_partitions | 整数 | 並列化可能な関係演算子の実行においてデータ分割を行う際のパーティション数。デフォルトは5。| この値はmax_result_set_writers 以下でなければならない|
...
| loader_path | String | UDF プラグイン (.so ファイル) を配置するディレクトリへのパス。デフォルトは /home/tsurugidb/plugins | |
| grpc_url | String | Tsurugi Database と UDF サーバ間の通信に利用する gRPC サーバの URL。デフォルトは "localhost:50051" | |
## ipc_endpointセクション
