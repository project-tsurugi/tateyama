...
|default_partitions | 整数 | 並列化可能な関係演算子の実行においてデータ分割を行う際のパーティション数。デフォルトは5。| 設定値はmax_result_set_writersより小さくなければならない|
...
|scan_default_parallel| 整数 | スキャンタスクの最大並列実行数。デフォルトは4。|この値はdefault_partitions及びmax_result_set_writersの値より小さくしなければならない。開発用のため将来的に削除/変更される可能性あり|
...
|max_result_set_writers | 整数 | writer resultsetの最大数。デフォルトは64。|この値は256以下でなけらばならない|
