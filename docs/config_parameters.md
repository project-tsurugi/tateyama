...
|busy_worker| ブール(true/false) | タスクスケジューラのワーカーが高頻度でタスクキューをチェックするか。enable_watcher=falseの場合はfalseであってはならない。デフォルトはfalse|開発用のため将来的に削除される可能性あり|
|enable_watcher| ブール(true/false) | タスクスケジューラで条件確認スレッドを使用するか。busy_worker=falseの場合はfalseであってはならない。デフォルトはtrue|開発用のため将来的に削除される可能性あり|
