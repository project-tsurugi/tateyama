...
* major SMV を変更する際、クライアント・サーバーともに後方互換性は考慮しなくてよい
  * 互換性を担保する場合、 **minor SMV を変更する** ようにする
  * ただし、breaking change (major SMV change) とクライアントの後方互換性 (サーバーだけアップグレードする場合) を両立するヒントについては [後方互換性](#後方互換性) のセクションに記載した
...
* [メッセージの変更とバージョニング](#メッセージの変更とバージョニング) において、以下の方法で後方互換性を保てる
  ...
  * (d') 必須リクエストフィールドの削除
    * フィールドの定義を削除すると後方互換性が失われるが、当該フィールドを残してクライアントが以降も (サービスにとって) 適切な値を設定し続ければ後方互換性を保てる
  * (e) 任意リクエストフィールドの追加
  ...
  * (g') 必須レスポンスフィールドの削除
    * フィールドの定義を削除すると後方互換性が失われるが、当該フィールドを残してサービスが以降も (クライアントにとって) 適切な値を設定し続ければ後方互換性を保てる
  * (h) 任意レスポンスフィールドの追加
    * minor SMV の変更なので問題ない
  * (h') 任意レスポンスフィールドの削除
    * minor SMV の変更なので問題ない
