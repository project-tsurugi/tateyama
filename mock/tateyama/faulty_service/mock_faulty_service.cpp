...
constexpr static std::string_view log_location_prefix = "/:tateyama:mock_faulty_service:logging ";

framework::component::id_type mock_faulty_service::id() const noexcept {
    return tag;
}

bool mock_faulty_service::setup(framework::environment&) {
    return true;
}

bool mock_faulty_service::start(framework::environment&) {
    return true;
}

bool mock_faulty_service::shutdown(framework::environment&) {
    return true;
}

mock_faulty_service::~mock_faulty_service() {
    VLOG(log_info) << "/:tateyama:lifecycle:component:<dtor> " << component_label;
}

bool mock_faulty_service::operator()(std::shared_ptr<request> req, [[maybe_unused]] std::shared_ptr<response> res) {
...
std::string_view mock_faulty_service::label() const noexcept {
