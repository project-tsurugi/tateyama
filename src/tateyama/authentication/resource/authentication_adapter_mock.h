...
class authentication_adapter_mock : public authentication_adapter {
  public:
    authentication_adapter_mock(bool enabled) : enabled_(enabled) {
        if (enabled_) {
            if (const char* key = std::getenv("TSURUGI_JWT_SECRET_KEY")) {
                rsa_ = std::make_unique<crypto::rsa>(key);
            } else {
                std::runtime_error("error in authentication setup: TSURUGI_JWT_SECRET_KEY is not provided by environment variable");
            }
        }
    }
    ~authentication_adapter_mock() = default;

    bool is_enabled() const noexcept override {
        return enabled_;
    }

    std::optional<encryption_key_type> get_encryption_key() const override {
        return std::make_pair("RSA" , rsa_->public_key());
    }

    std::optional<std::string> verify_token(std::string_view token) const override {
    ...
    std::optional<std::string> verify_encrypted(std::string_view username, std::string_view password) const override {
