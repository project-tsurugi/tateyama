...
public:
    ...
    void decrypt(std::string_view in, std::string &out) {
        ...
        if (EVP_PKEY_decrypt(ctx.get(), nullptr, &buf_len, static_cast<const unsigned char*>(static_cast<const void*>(in.data())), in.length()) <= 0) {
...
private:
    ...
    EVP_PKEY *get_key(const std::string& pem_key_string, int selection) {
        ...
        const auto *data = static_cast<const unsigned char*>(static_cast<const void*>(pem_key_string.data()));
