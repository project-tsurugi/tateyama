...
public:
    ...
    void decrypt(std::string_view in, std::string &out) {
        ...
        if (EVP_PKEY_decrypt(ctx.get(), nullptr, &buf_len, reinterpret_cast<const unsigned char*>(in.data()), in.length()) <= 0) {
            throw std::runtime_error("fail to EVP_PKEY_decrypt first");
        }
        out.resize(buf_len);
        if (EVP_PKEY_decrypt(ctx.get(), reinterpret_cast<unsigned char*>(out.data()), &buf_len, reinterpret_cast<const unsigned char*>(in.data()), in.length()) <= 0) {
...
private:
    ...
    EVP_PKEY *get_key(const std::string& pem_key_string, int selection) {
        ...
        const auto *data = reinterpret_cast<const unsigned char*>(pem_key_string.data());
