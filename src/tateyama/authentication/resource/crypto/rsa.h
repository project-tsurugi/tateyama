...
public:
    ...
    void decrypt(std::string_view in, std::string &out) {
        ...
        if (EVP_PKEY_decrypt_init_ex(ctx.get(), params) <= 0) {
            throw std::runtime_error("fail to EVP_PKEY_decrypt_init_ex");
        }
        if (EVP_PKEY_decrypt(ctx.get(), NULL, &buf_len, static_cast<const unsigned char*>(static_cast<const void*>(in.data())), in.length()) <= 0) {
            throw std::runtime_error("fail to EVP_PKEY_decrypt first");
        }
        out.resize(buf_len);
        if (EVP_PKEY_decrypt(ctx.get(), static_cast<unsigned char*>(static_cast<void*>(out.data())), &buf_len, static_cast<const unsigned char*>(static_cast<const void*>(in.data())), in.length()) <= 0) {
...
private:
    ...
    void set_public_key() {
        ...
        if (!i2d_PUBKEY_bio(out.get(), evp_private_key_.get())) {
    ...
    EVP_PKEY *get_key(const std::string& pem_key_string, int selection) {
        EVP_PKEY *pkey = NULL;
        const unsigned char *data = static_cast<const unsigned char*>(static_cast<const void*>(pem_key_string.data()));
        size_t data_len = pem_key_string.length();

        auto dctx = make_handle(OSSL_DECODER_CTX_new_for_pkey(&pkey, "PEM", NULL, "RSA",
