...
class alive_timer {
public:
    alive_timer(double refresh, std::size_t timeout) :
        refresh_(static_cast<std::size_t>(lround(refresh * 10000000))), timeout_(timeout), enable_timeout_(timeout > 0) {
        if (enable_timeout_) {
...
private:
    std::chrono::microseconds refresh_;
