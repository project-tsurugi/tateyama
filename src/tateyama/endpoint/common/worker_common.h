...
protected:
    ...
    bool handshake(tateyama::api::server::request* req, tateyama::api::server::response* res) {
        ...
        if (auth_) {
            ...
            case tateyama::proto::endpoint::request::Credential::CredentialOptCase::kFileCredential:
            {
                if (auto username_opt = auth_->verify_encrypted(credential.file_credential()); username_opt) {
