...
public:
    ipc_response(ipc_request& request, std::size_t index, std::function<void(std::chrono::nanoseconds)> mark_end) :
    ...
    ipc_response(ipc_request& request, std::size_t index) : ipc_response(request, index, [](std::chrono::nanoseconds){}) {}
...
private:
    ...
    std::function<void(std::chrono::nanoseconds)> mark_end_;
    ...
    void mark_end() {
        ...
        mark_end_(std::chrono::duration_cast<std::chrono::nanoseconds>(now - time_));
    }
};
