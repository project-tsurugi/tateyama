...
bool system_service_core::operator()(const std::shared_ptr<request>& req, const std::shared_ptr<response>& res) {
    try {
        ...
        case tateyama::proto::system::request::Request::kGetDatabaseProductVersion:
        {
            tateyama::proto::system::response::GetDatabaseProductVersion rs{};
            try {
                std::string version = info_handler_.get_version();
                if (!version.empty()) {
                    rs.mutable_success()->set_product_version(version);
            ...
            auto* error = rs.mutable_error();
            error->set_code(tateyama::proto::system::diagnostic::ErrorCode::NOT_FOUND);
            error->set_message("cannot determine product version");
            res->body(rs.SerializeAsString());
            return true;
        }
