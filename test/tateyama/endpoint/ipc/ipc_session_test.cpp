...
class ipc_session_test : public ::testing::Test {
    virtual void SetUp() {
        ...
        worker_ = std::make_unique<tateyama::endpoint::ipc::bootstrap::Worker>(service_, my_session_id, wire, database_info_, nullptr);
...
TEST_F(ipc_session_test, cancel) {
    std::this_thread::sleep_for(std::chrono::milliseconds(500));

    // client part (send request)
    auto client = std::make_unique<ipc_client>(database_name, my_session_id);
