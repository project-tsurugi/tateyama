...
public:
    ...
    void operator()() {
        while (true) {
            ...
            if (stream != nullptr) {
                worker_ = std::make_unique<tateyama::endpoint::stream::bootstrap::stream_worker>(service_, my_session_id_, std::move(stream), database_info_, [](tateyama::endpoint::stream::bootstrap::stream_worker*){});
