...
class blob_relay_service_adapter_test :
    public ::testing::Test,
...
public:
    void SetUp() override {
        ...
        service_adapter_ = std::dynamic_pointer_cast<grpc::blob_relay::service_adapter>(sv_->server::find_resource_by_id(framework::resource_id_blob_relay_service));
        if (!service_adapter_) {
            FAIL();
        }
        ...
        try {
            blob_relay_service_ = service_adapter_->blob_relay_service();
...
protected:
    std::shared_ptr<grpc::blob_relay::service_adapter> service_adapter_{};
...
TEST_F(blob_relay_service_adapter_test, basic) {
    auto& blob_session = blob_relay_service_->create_session();
    blob_session.dispose();
}

TEST_F(blob_relay_service_adapter_test, add_blob) {
    auto& blob_session = blob_relay_service_->create_session();
...
TEST_F(blob_relay_service_adapter_test, add_blob_no_file) {
    auto& blob_session = blob_relay_service_->create_session();
...
TEST_F(blob_relay_service_adapter_test, entries_and_remove) {
    auto& blob_session = blob_relay_service_->create_session();
